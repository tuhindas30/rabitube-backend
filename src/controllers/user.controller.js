...
const getAllUsers = async (req, res, next) => {
  try {
    const users = await User.find({}).populate("watchlater").populate("liked").populate("playlists.videos");
...
const findUser = async (req, res, next, userId) => {
  try {
    const user = await User.findById(userId).populate("watchlater").populate("playlists.videos").populate("liked");
    if (!user) {
